...
class LitNodeClientNodeJs extends core_1.LitCore {
    ...
    constructor(args) {
        ...
        this.createCapacityDelegationAuthSig = async (params) => {
            const { dAppOwnerWallet, capacityTokenId, delegateeAddresses, uses, domain, expiration, statement, } = params;
            ...
            const capabilities = {
                ...
                ...(delegateeAddresses
                    ? {
                        delegate_to: delegateeAddresses.map((address) => address.startsWith('0x') ? address.slice(2) : address),
                    }
                    : {}),
                uses: _uses.toString(),
            };
            ...
            // -- get auth sig
            let siweMessage = new siwe_1.SiweMessage({
                domain: _domain,
    ...
    `);
        ...
        this.signSessionKey = async (params) => {
            ...
            const pkpEthAddress = (function () {
                if (params.pkpPublicKey) {
                    ...
                    const hexedPkpPublicKey = (0, misc_1.hexPrefixed)(params.pkpPublicKey);
                    if (hexedPkpPublicKey)
                        return (0, transactions_1.computeAddress)(hexedPkpPublicKey);
                }
        ...
        this.getSessionSigs = async (params) => {
            ...
            const sessionExpiration = params.expiration ?? new Date(Date.now() + 1000 * 60 * 5).toISOString();
    ...
    async claimKeyId(params) {
        ...
        const responseData = await (0, misc_1.executeWithRetry)(wrapper, (_error, _requestId, isFinal) => {
            if (!isFinal) {
                (0, misc_1.logError)('an error occured, attempting to retry');
            }
        }, this.config.retryTolerance);
